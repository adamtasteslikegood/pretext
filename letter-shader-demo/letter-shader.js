...
function findGlyph(targetB, targetW) {
  let lo = 0, hi = glyphs.length - 1;
  while (lo < hi) {
    const mid = lo + hi >> 1;
    if (glyphs[mid].brightness < targetB)
      lo = mid + 1;
    else
      hi = mid;
  }
...
function gauss(v, sigma) {
  return Math.exp(-0.5 * (v * v) / Math.max(sigma * sigma, 0.0001));
}
function sampleField(cx, cy, cols, rows, time) {
  const ux = cx / cols * 2 - 1;
  let uy = cy / rows * 2 - 1;
  uy += 0.06 * Math.sin(ux * 2.4 - time * 0.62) + 0.026 * Math.sin(ux * 6.8 + time * 0.44);
  const center = -0.06 + 0.22 * Math.sin(ux * 1.18 - time * 0.46) + 0.07 * Math.sin(ux * 3.75 + time * 0.28) + 0.12 * Math.exp(-5.4 * (ux - 0.78) ** 2) * Math.sin(time * 0.72 + ux * 4.6) - 0.08 * Math.exp(-6.5 * (ux + 0.62) ** 2) * Math.cos(time * 0.54 - 0.3);
  const thickness = 0.84 + 0.12 * Math.sin(ux * 1.6 + time * 0.18) + 0.05 * Math.cos(ux * 4.8 - time * 0.22);
  ...
  const fold = 0.11 * Math.sin(ux * 7.2 - time * 0.78 + local * 6.2) + 0.04 * Math.sin(ux * 13.4 + time * 0.41 - local * 8.6);
...
function paletteColor(pal, t, alpha) {
  ...
  for (const pal of PALETTES) {
    ...
    btn.addEventListener("click", () => {
      ...
      for (const b of paletteGridEl.querySelectorAll(".palette-button")) {
        b.setAttribute("aria-pressed", String(b.dataset["paletteId"] === pal.id));
      }
    });
